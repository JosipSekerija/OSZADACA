...
//Modelirati ringiØpil  s dva tipa dretvi/procesa:dretvama posjetitelji koji zele na voznju i vrtuljak.Napisati program koji nece dozviliti posjetiteljima da se ukrcaju kada vise nema mjesta i da vrtuljak pokrenemo tek kada je pun.Za sinkronizaciju korisiti opce semafore i dodatne varijable.
...
sem_t mutex; // Semafor za mešusobno iskljucivanje
...
void* posjetitelj(void* arg) {
    ...
    sem_wait(&mutex); // Uši u kriticni odsjecak
    seats[count] = id;
    count++;
...
void* vrtuljak(void* arg) {
    while (1) {
        ...
        sem_post(&mutex); // Izadi iz kritičnog odsječka
